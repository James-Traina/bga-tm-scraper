#!/usr/bin/env python3
"""
Test script for the unified Terraforming Mars parser
"""
import os
import json
import logging
...
def test_unified_parser():
    """Test the unified parser with the existing game data"""
    ...
    # Parse the game
    print(f"🔄 Parsing game with unified parser...")
    ...
    try:
        ...
        # Export to JSON
        output_file = "data/parsed/unified_game_250604-1037.json"
...
def analyze_unified_data():
    """Analyze the unified game data"""
    
    output_file = "data/parsed/unified_game_250604-1037.json"
    
    if not os.path.exists(output_file):
        print(f"❌ Unified data file not found: {output_file}")
        print("Run the parser first.")
        return
    
    print(f"📊 Analyzing unified game data...")
    
    with open(output_file, 'r', encoding='utf-8') as f:
        data = json.load(f)
    
    print(f"✅ Loaded unified data")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == 'analyze':
        ...
        game_data = test_unified_parser()
        if game_data:
            print(f"\n🎉 Unified parsing test completed successfully!")
            print(f"   Run 'python test_unified_parser.py analyze' to analyze the data")
